...
export default function govukSkipLink(params) {
  return html`<a href="${params.href || '#content'}" class="govuk-skip-link${(params.classes && ` ${params.classes}` || '')}"!${govukAttributes(params.attributes)} data-module="govuk-skip-link">!${params.html ?? html`${params.text}`}</a>`
}
